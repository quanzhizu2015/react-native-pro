...
import {appState, mnemonicLogin, privateKeyLogin, setToken} from '../comm/sdk';
...
export default class PLogin extends Component{
    ...
    login= async() =>{
        ...
        if(!this.state.private_key.trim()){
            Toast.show({
                text:appState.lan.mnemonic.tip11,
                position: 'center',
            })
        ...
        privateKeyLogin({private_key:this.state.private_key,device_no:Device_No}).then((respond)=>{
            if (respond.code==200){
    ...
    render(){
        return <View style={{paddingTop:23}}>
            ...
            <Input style={{width:screenW-50,height:224,marginTop:22,backgroundColor:'#19191E',borderColor:'#ffffff00',color:"#fff",fontSize:14,fontFamily:FONT_R}}
                   ...
                   onChangeText={text => this.setState({private_key: text})}
